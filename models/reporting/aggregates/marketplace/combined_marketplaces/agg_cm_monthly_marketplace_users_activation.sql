...
FROM (
    ...
    FROM {{ ref('int_activation_monthly_activations_list') }} -- Referencing a DBT model for monthly user activations
    GROUP BY year_month, activation_type, partner_marketplace -- Grouping the data by year_month and activation type to prepare for aggregation
...
GROUP BY year_month, partner_marketplace
