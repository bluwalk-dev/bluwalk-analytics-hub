...
FROM (
    ...
    WHERE ymk.end_date <= current_date -- Ensures the data is up to the current date
) a
LEFT JOIN {{ ref('fct_user_activity') }} b 
    ON b.year_month = a.next_year_month -- Matches to check if the user is active in the subsequent month
    AND a.lost_user_id = b.user_id -- Ensures the same user is compared across months
WHERE b.user_id IS NULL -- Filters to include only users who are not active in the subsequent month
AND a.lost_user_id IS NOT NULL -- Excludes records without a valid user ID
