select
    ...
    datetime_diff(
        if(
            a.property_closed_date is null,
            current_timestamp(),
            a.property_closed_date
        ),
...
from {{ ref("stg_hubspot_ticket") }} a
left join {{ ref("stg_hubspot__ticket_pipeline_stages") }} c on a.property_hs_pipeline_stage = cast(c.stage_id as int)
left join {{ ref("dim_employees__version2") }} e on CAST(a.property_hs_all_owner_ids AS INT64) = e.employee_hubspot_owner_id
left join {{ ref("dim_users__version2") }} d on cast(a.property_odoo_user_id as int64) = d.user_id
