...
transformation as (
    ...
    SELECT
        ...
        posto as station_id,
        tomada as plug_id,
        TIMESTAMP(PARSE_DATETIME('%d/%m/%Y %H:%M:%S', dataInicio), 'Europe/Lisbon') as start_timestamp,
        PARSE_DATETIME('%d/%m/%Y %H:%M:%S', dataInicio) AS start_date,
        TIMESTAMP(PARSE_DATETIME('%d/%m/%Y %H:%M:%S', dataFim), 'Europe/Lisbon') as stop_timestamp,
        PARSE_DATETIME('%d/%m/%Y %H:%M:%S', dataFim) AS stop_date,
