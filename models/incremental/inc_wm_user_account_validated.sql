...
WITH uberDriverStatus AS (
    SELECT
        ...
        DATETIME(TIMESTAMP_MILLIS(MIN(load_timestamp)), 'Europe/Lisbon') as account_validated_date  -- The earliest timestamp converted to a date indicating when the account was validated
...
FROM {{ ref("base_hubspot_deals") }} a  -- Reference to the base deals model
...
WHERE
  ...
  c.driver_uuid IS NOT NULL  -- Ensuring that there is a matched driver record