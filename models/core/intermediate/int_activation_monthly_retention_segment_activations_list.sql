...
SELECT * FROM (
    ...
    FROM {{ ref('agg_retention_segments_daily_activity') }} -- Reference to a user activity fact table
    GROUP BY user_id, contact_id, retention_segment
    ...
    FROM 
        ...
        (SELECT 
            ...
            retention_segment,
            year_month
        FROM {{ ref('agg_retention_segments_daily_activity') }}) au
        ...
        (SELECT 
            ...
            retention_segment,
            user_id 
        FROM {{ ref('int_retention_monthly_retention_segment_churns_list') }}) lcu
    WHERE 
        ...
        au.retention_segment = lcu.retention_segment
    
    GROUP BY lcu.year_month, au.user_id, au.contact_id, au.retention_segment
